...
#+BEGIN_SRC sh
export DOCKER_HOST=52.3.241.136:2375
#+END_SRC

First we need to copy the configuration file to the docker host.
#+BEGIN_SRC sh
scp -i ~/.ssh/ec2/us-east-1.pem docker-double-divide-aws-leader.ini ubuntu@52.3.241.136:/tmp
#+END_SRC
...
docker run --rm -u 0 \
 ...
 -w /tmp \
 --net=buffy-leader \
 docker.sendence.com:5043/sendence/dagon.amd64:sendence-2.3.0-555-ga154bee \
 dagon.amd64 \
 --docker=tcp://10.0.30.128:2375  -t 10 \
...
Point your =DOCKER_HOST= env variable to the AWS leader on port 2375.
#+BEGIN_SRC sh
export DOCKER_HOST=10.0.30.128:2375
#+END_SRC

First we need to copy the configuration file to the docker host.
#+BEGIN_SRC sh
scp -i ~/.ssh/ec2/us-east-1.pem docker-double-divide-aws-swarm.ini ubuntu@52.3.241.136:/tmp
#+END_SRC
...
docker run --rm -u 0 \
 ...
 docker.sendence.com:5043/sendence/dagon.amd64:sendence-2.3.0-555-ga154bee \
 dagon.amd64 \
 --docker=tcp://10.0.30.128:2378  -t 10 \
