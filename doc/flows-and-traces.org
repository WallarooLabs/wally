...
A join implies that the receiving worker blocks until it receives all the
indicated inputs. So in the above example, $D$ waits until it receives inputs
from both $B$ and $C$ before it processes the data. However, sometimes we might
want $D$ to start working and sending along outputs as soon as it receives from
$B$ or $C$. For this we have the *nonblocking join operation*, which is indicated
by an overline followed by a carat:

\begin{center}
$A;\overline{B||C}\string^;D$
\end{center}

This indicates that $D$ should still eventually receive data from both $B$ and $C$,
but that it doesn't need to wait for both before it starts processing.
...
Note the use of parens to group the conjuncts of the initial fork.

In some cases we might want to indicate that data should flow back to a worker
that has showed up earlier. For example, imagine a case where $A$ sends a question to
$B$ and then sends a message to $C$ once it receives a reply. For example:
...
We can represent this by using a subscript to identify that we are reusing a
worker in our $Flow$:
...
Our model for expressing flows cannot capture every case as it stands. Consider the following:
...
of overlines for joins. For example, consider a more complex example from the
last section:
...
can (must notationwise) be thought of as:
