...
primitive BYOIFramedSourceNotifyBuilder[In: Any val]
  fun apply(source_id: StepId, pipeline_name: String, env: Env,
    ...
    SourceNotify iso^
  =>
    BYOIFramedSourceNotify[In](source_id, pipeline_name, env, auth, handler,
      runner_builder, router, consume metrics_reporter, event_log,
      target_router, pre_state_target_ids)

class BYOIFramedSourceNotify[In: Any val] is BYOISourceNotify
  ...
  fun ref received(source: BYOISource ref, data: Array[U8] iso): Bool =>
    if _header then
      try
      ...
      (let is_finished, let last_ts) =
        try
          ...
          _metrics_reporter.step_metric(_pipeline_name,
            "Decode Time in BYOI Source", latest_metrics_id, ingest_ts,
            decode_end_ts)
      ...
      if is_finished then
        ...
        ifdef "detailed-metrics" then
          _metrics_reporter.step_metric(_pipeline_name,
            "Before end at BYOI Source", 9999,
            last_ts, end_ts)
        end
  ...
  fun ref accepted(source: BYOISource ref) =>
    @printf[I32]((_source_name + ": accepted a connection\n").cstring())
    source.expect(_header_size)

  fun ref closed(source: BYOISource ref) =>
    @printf[I32]("BYOISource connection closed\n".cstring())
