...
class DataRouter
  ...
  fun route(d_msg: DeliveryMsg val, origin: Producer, seq_id: SeqId, latest_ts: U64, metrics_id: U16) =>
    ifdef "trace" then
  ...
  =>
    try
      ...
      r_msg.replay_deliver(_data_routes(target_id), origin, seq_id, latest_ts, metrics_id)
      false
    else
      true
    end
