...
actor Main
  new create(env: Env) =>
    try
      ...
      let word_totals_partition_2 = Partition[RunningTotal, String](
        WordPartitionFunction, PartitionFileReader("letters.txt",
          env.root as AmbientAuth))
      ...
      let application = recover val
        Application("Word Count App")
          .new_pipeline[String, RunningTotal]("Word Count",
            ...
            .to_state_partition[RunningTotal, String, RunningTotal,
              WordTotals](
              AddCount, WordTotalsBuilder, "word-totals-2",
              word_totals_partition_2 where multi_worker = true)
...
class val RunningTotal is (Hashable & Equatable[RunningTotal])
  let word: String
  let count: U64
...
primitive AddCount is StateComputation[(String | RunningTotal), RunningTotal, WordTotals]
  fun name(): String => "Add Count"

  fun apply(w: (String | RunningTotal),
...
primitive WordPartitionFunction
  fun apply(input: (String | RunningTotal)): String =>
    try
      let key =
        match input
        | let s: String => s
