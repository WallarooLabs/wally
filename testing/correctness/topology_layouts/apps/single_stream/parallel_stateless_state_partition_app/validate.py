...
parser = argparse.ArgumentParser('Parallel Stateless -> State Partition validator')
...
# Split on partition (mod 6)
processed = [[], [], []]
for c,m in received:
    processed[(m % 6)/2].append((c,m))
# Sort on max
...
for v in range(1, args.n+1):
    v2 = v * 2  # double
    i = (v2 % 6)/2  # partition mod 6
    maxes[i] = v2   # set max for partition
