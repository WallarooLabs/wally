  ...
  new create(env: Env) =>
    try
      ...
      let application = recover val
        Application("single_stream-parallel_stateless_state_partition_app")
          .new_pipeline[U64, U64]("U64 Double CountAndMax",
            TCPSourceConfig[U64].from_options(U64Decoder,
              TCPSourceConfigCLIParser(env.args)?(0)?))
            .to_parallel[U64]({(): Double => Double})
            .to_state_partition[U64 val, U64 val, CountMax, CountAndMax](
      ...
      Startup(env, application,
        "single_stream-parallel_stateless_state_partition_app")
    else
