...
Since our Filtering Stateless computation is an OddFilter, the values will be
filtered if odd and written to file as a framed message to compare the
...
actor Main
  new create(env: Env) =>
    ...
    for i in Range[I64](0, message_count) do
      ...
      | (let n: U64) =>
        match exp_file
        | (let e: File) => e.writev(FramedU64Encoder(n, wb))
        end
      end
    end
