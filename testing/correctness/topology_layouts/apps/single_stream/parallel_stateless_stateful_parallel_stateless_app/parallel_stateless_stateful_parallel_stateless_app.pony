  ...
  new create(env: Env) =>
    try
      let application = recover val
        Application("single_stream-parallel_stateless_stateful_parallel_stateless_app")
          .new_pipeline[U64, U64]("U64 Double Sum Divide",
            ...
            .to_parallel[U64]({(): Divide => Divide})
            .to_sink(TCPSinkConfig[U64].from_options(
              FramedU64Encoder,
              TCPSinkConfigCLIParser(env.args)?(0)?))
      end
      Startup(env, application,
        "single_stream-parallel_stateless_stateful_parallel_stateless_app")
    else
