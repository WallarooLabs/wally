...
parser = argparse.ArgumentParser('Stateful -> Parallel Stateless -> Stateful validator')
...
expected = args.n * (args.n+1)

try:
    assert(expected == processed[-1][1])
    assert(args.n == len(processed))
    assert(args.n == processed[-1][0])
