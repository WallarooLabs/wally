...
class Ring[A: Any val]
  ...
  fun to_array(): Array[A] iso^ =>
    """
    Get an Array[A] iso of the current ring buffer values with the most recent
    value at the end.
    """
    let ar: Array[A] iso = recover Array[A](size()) end
    for v in values() do
      ar.push(v)
    end
    ar.reverse_in_place()
  ...
  fun clone(): Ring[A] iso^ =>
    """
    Clone the Ring. The new Ring contains references to the same elements that
    the old Ring contains, the elements themselves are not cloned.
    """
    (let buf', let size', let count') = raw()
    recover Ring[A].from_array(consume buf', size', count') end
