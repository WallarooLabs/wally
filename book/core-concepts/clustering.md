...
Imagine for a moment you have a Wallaroo Python application like our [Alphabet Partitioned example](https://github.com/WallarooLabs/wallaroo/tree/release-0.1.2/examples/python/alphabet_partitioned). Alphabet Partitioned starts up a 2 worker cluster. However, it does it on a single machine so it might be less than obvious what the specific IP addresses are that we are passing.
...
Wallaroo comes with a [cluster shutdown tool](https://github.com/WallarooLabs/wallaroo/tree/release-0.1.2/utils/cluster_shutdown) that can be used to shut down a running cluster.
