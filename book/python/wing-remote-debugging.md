...
```bash
cd ~/wallaroo-tutorial/wallaroo/book/examples/python/reverse
```
...
Now that our project is setup, let's open the `~/wallaroo-tutorial/wallaroo/book/examples/python/reverse/wingdbstub.py` file via Wing's `File -> Open Remote File`.
...
We'll also open the `~/wallaroo-tutorial/wallaroo/book/examples/python/reverse/reverse.py` file via Wing's `File -> Open Remote File`
...
Now that we're setup, let's run our `reverse` example. You can find the instructions [here](/book/examples/python/reverse/README.d).
