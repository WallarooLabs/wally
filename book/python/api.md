...
For an overview of what aggregations are and how they must be implemented, see [here](/book/core-concepts/aggregations.md). 
...
For an in-depth overview of windows, see [here](/book/core-concepts/windows.md). Windows are either count-based or range-based.
...
`with_slide(slide)`: specifies a slide for these windows, resulting in sliding (overlapping) windows. For example, if the slide is 3 seconds, then a new window of length `range` (specified via `wallaroo.range_windows(range)`) will be started every 3 seconds.

`with_delay(delay)`: specifies a delay on triggering a window. This is either (1) an estimation of the maximum lateness expected for messages or (2) the lateness threshold beyond which you no longer care about messages.
