defmodule MetricsReporterUI.IngressEgressChannelTest do
	use MetricsReporterUI.ChannelCase
  require Logger

  alias MetricsReporterUI.IngressEgressChannel
  alias MonitoringHubUtils.MessageLog

  setup do
    throughput_msg_log_name = "category:ingress-egress::cat-name:NODE12::total-throughput:last-1-sec"
    ...
    throughput_msg1 = %{"total_throughput" => 100, "time" => timestamp1, "pipeline_key" => "NODE12"}
    ...
    {:ok, _, socket} = socket()
      |> subscribe_and_join(IngressEgressChannel, "ingress-egress:NODE12")

    {:ok, socket: socket, throughput_msg1: throughput_msg1, throughput_msg2: throughput_msg2}
  end

  test "sends 'initial-total-throughputs' after joining", %{throughput_msg1: throughput_msg1} do
    payload = %{data: [throughput_msg1], pipeline_key: "NODE12"}
